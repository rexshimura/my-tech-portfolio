...
export default function ExpandSelectedProject({ project, isOpen, onClose }) {
  ...
  const hasValidThumbnail = project.thumbnail && project.thumbnail.trim() !== "";
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 md:p-8">
      ...
      >
        ...
        {/* =========================================================================
            STATE A: FULL ASSET IMMERSIVE PREVIEW MODE (WITH MAGIC LAYOUT ANIMATION)
            ========================================================================= */}
        <AnimatePresence>
          {fullscreenMedia && (
            <motion.div 
              ...
              className="absolute inset-0 w-full h-full flex flex-col p-5 sm:p-8 bg-white/95 dark:bg-zinc-950/95 z-40"
            >
              ...
              <motion.div 
                ...
                className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-white/5 shrink-0"
              >
                <button 
                  onClick={() => setFullscreenMedia(null)}
                  className="cursor-target sfx-bop flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 hover:text-blue-500 dark:hover:text-white transition-colors bg-gray-100 dark:bg-zinc-900 rounded-lg cursor-pointer"
                >
                ...
                <button 
                  onClick={() => setFullscreenMedia(null)}
                  className="cursor-target sfx-bop p-2 rounded-full bg-gray-100 dark:bg-zinc-900 text-gray-500 dark:text-gray-400 hover:text-red-500 transition-colors cursor-pointer"
              ...
              <div className="flex-1 w-full bg-transparent flex items-center justify-center overflow-hidden pt-4">
                {isVideoFile(fullscreenMedia) ? (
                  <motion.video 
                    ...
                    controls 
                    autoPlay 
                    className="max-w-full max-h-full rounded-xl shadow-2xl border border-gray-100 dark:border-white/5 object-contain" 
                  />
                ) : (
                  <motion.img 
                    layoutId={`media-${fullscreenMedia}`} // <--- MAGIC MORPH LINK
        ...
        <div 
          className={`w-full h-full overflow-y-auto p-5 sm:p-8 space-y-6 custom-scrollbar transition-all duration-300 ease-in-out ${
            fullscreenMedia ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'
          }`}
        >
          ...
          <button 
            onClick={onClose}
            className="cursor-target sfx-bop absolute right-5 top-5 z-30 p-2.5 rounded-full bg-gray-100 dark:bg-zinc-900 text-gray-800 dark:text-white hover:text-red-500 transition-colors active:scale-90 cursor-pointer"
          >
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start pt-2">
            ...
            {/* CLICKABLE MAIN THUMBNAIL */}
            <div 
              onClick={() => hasValidThumbnail && setFullscreenMedia(project.thumbnail)}
              className={`w-full md:w-[260px] aspect-video rounded-xl bg-gray-100 dark:bg-zinc-900 border border-gray-200/60 dark:border-white/5 overflow-hidden shrink-0 relative group select-none transition-all duration-200 ${
                hasValidThumbnail ? 'cursor-target sfx-mini cursor-zoom-in hover:border-blue-500/30' : 'cursor-default flex items-center justify-center'
              }`}
            >
              {hasValidThumbnail ? (
                <>
            ...
            <div className="space-y-3 flex-1 pr-6 sm:pr-0 w-full">
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-2 w-full max-w-md">
                ...
                {hasCode ? (
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="cursor-target sfx-bop flex-1 py-2 px-4 rounded-lg border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 font-bold text-[10px] uppercase tracking-widest hover:bg-gray-50 dark:hover:bg-white/5 transition-colors flex items-center justify-center gap-1.5 active:scale-98"
                  >
                    <Github size={14} /> Source Repository
                  </a>
                ) : (
                  <div className="flex-1 py-2 px-4 rounded-lg border border-dashed border-gray-200 dark:border-white/5 text-gray-400 dark:text-zinc-600 font-bold text-[10px] uppercase tracking-widest text-center select-none bg-gray-50/50 dark:bg-transparent">
                    Private Source
                  </div>
                )}
              </div>
                ...
                return (
                  <div 
                    ...
                    className={`aspect-video rounded-lg bg-gray-50 dark:bg-zinc-900/40 border border-gray-200/60 dark:border-white/5 overflow-hidden flex items-center justify-center relative group select-none ${
                      hasValidMedia ? 'cursor-target sfx-mini cursor-zoom-in hover:border-blue-500/30' : 'cursor-default'
